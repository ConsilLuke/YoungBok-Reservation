...
def page_reserve():
    ...
    # 📌 사전 허가 안내
    st.subheader("5️⃣ 특별실 예약 안내")
    
    st.info("""
**📌 특별실 예약 안내**

1. 특별실 사용을 원하는 경우, 반드시 해당 특별실 담당 선생님께 사전 허가(구두, 메신저 등)를 받아야 합니다.

2. 담당 선생님의 허가 없이 등록된 예약은 관리자에 의해 사전 통보 없이 취소될 수 있습니다.

3. 예약 후 일정 변경이 필요한 경우, '내 예약'에서 취소 후 재예약하시기 바랍니다.
    """)
    
    confirmed = st.checkbox("✅ 위 안내사항을 확인하였으며, 특별실 담당 선생님께 사전 허가를 받았습니다.")
    ...
    if st.button("🎯 예약 신청", use_container_width=True, type="primary"):
        if not selected_periods:
            ...
            st.error("특별실 담당 선생님께 사전 허가를 받았는지 확인해주세요.")
        else:
            success_count = 0
            fail_messages = []
